    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, _) in enumerate(loader):
            ...
            if (batch_idx + 1) % print_interval == 0 or batch_idx == 0:
                bar.suffix = '({batch}/{size}) top1: {top1:.4f} | top5: {top5:.4f}'.format(
                             batch=batch_idx + 1,
                             size=len(loader),
                             top1=top1.avg,
                             top5=top5.avg,
...
def main():
    ...
    for gen_idx in range(start_gen, args.num_generation):
        ...
        if args.dataset == 'cifar10':
            base_dataset = torchvision.datasets.CIFAR10('./data/cifar-10', train=True, download=False)
            test_set = torchvision.datasets.CIFAR10('./data/cifar-10', train=False, download=False, transform=tfms_test_cifar10)
            weakDA = tfms_w_cifar10
            strongDA = tfms_s_cifar10
            valDA = tfms_test_cifar10
        elif args.dataset == 'cifar100':
            base_dataset = torchvision.datasets.CIFAR100('./data/cifar-100', train=True, download=False)
            test_set = torchvision.datasets.CIFAR100('./data/cifar-100', train=False, download=False, transform=tfms_test_cifar100)
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, scheduler, ema_optimizer, criterion, epoch, use_cuda,
    ...
    for batch_idx in range(args.val_iteration):
        ...
        if (batch_idx + 1) % print_interval == 0 or batch_idx == 0:
            bar.suffix = '({batch}/{size}) Loss: {loss:.4f} | Loss_x: {loss_x:.4f} | Loss_u: {loss_u:.4f} | ' \
                         'Loss_r: {loss_r:.4f} | Loss_e: {loss_e:.4f} | Total_acc: {total_c:.4f}'.format(
                batch=batch_idx + 1,
    ...
    print(f'  Epoch [{epoch+1}] - Loss: {losses.avg:.4f} | Loss_x: {losses_x.avg:.4f} | Loss_u: {losses_u.avg:.4f} | '
          f'Loss_r: {losses_r.avg:.4f} | Loss_e: {losses_e.avg:.4f} | Total_acc: {total_c.avg:.4f}')
