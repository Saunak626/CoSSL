...
def main():
    ...
    if args.dataset == 'cifar10':
        train_labeled_set, train_unlabeled_set, test_set, train_strong = dataset_cifar10.get_cifar10('./data/cifar-10',
                                                                                       N_SAMPLES_PER_CLASS,
                                                                                       U_SAMPLES_PER_CLASS,
                                                                                       return_strong_labeled_set=True, seed=args.manualSeed)
    elif args.dataset == 'cifar100':
        train_labeled_set, train_unlabeled_set, test_set, train_strong = dataset_cifar100.get_cifar100('./data/cifar-100',
                                                                                         N_SAMPLES_PER_CLASS,
                                                                                         U_SAMPLES_PER_CLASS,
                                                                                         return_strong_labeled_set=True, seed=args.manualSeed)
    else:
...
def train(labeled_trainloader, unlabeled_trainloader, model, ema_model, optimizer, ema_optimizer,
    ...
    for batch_idx in range(args.val_iteration):
        ...
        if (batch_idx + 1) % print_interval == 0 or batch_idx == 0:
            bar.suffix = '({batch}/{size}) Loss: {loss:.4f} | Loss_x: {loss_x:.4f} | Loss_u: {loss_u:.4f} | Loss_t: {loss_t:.4f} | ' \
    ...
    print(f'  Epoch [{epoch+1}] - Loss: {losses.avg:.4f} | Loss_x: {losses_x.avg:.4f} | Loss_u: {losses_u.avg:.4f} | '
          f'Loss_t: {losses_teacher.avg:.4f} | Mask: {mask_prob.avg:.4f} | Total_acc: {total_c.avg:.4f} | '
          f'Use_acc: {used_c.avg:.4f} | teacher_acc: {teacher_acc.avg:.4f}')
